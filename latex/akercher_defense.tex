...
%%  Title page
%%------------------------------------------------------------
\title[Computational MHD]{Removal of pseudo-convergence in near-coplanar Riemann problems of ideal magnetohydrodynamics \\ and \\ parallel fluid simulations on shared memory processors}
...
\institute[GMU]{George Mason University}
...
\subsection{Shocks in space plasma}

\begin{frame}[fragile]
\frametitle{Shocks in space plasma}
...
\pause
\item Plasma flow becomes subsonic forming a bow shock.
\ebi
}
\only<4->{
...
%----------------------------------------------------------------------------------------
\begin{frame}[fragile]
\frametitle{Limiting numerical diffusion}
%% \begin{columns}
%% \begin{column}{0.5\textwidth}
%% \only<1-1>{
%% \begin{figure}[htbp]
...
% Ideal MHD
%----------------------------------------------------------------------------------------
\section{Riemann problems of ideal MHD}
...
%% %% \begin{column}{0.5\textwidth}
%% \bei
%% \item HLLD \citep{Miyoshi:2005} (D for \emph{discontinuities}) extension of HLLC to MHD.
%% \pause
...
\only<2-2>{
\begin{figure}[htbp] 
\resizebox{0.5\linewidth}{!}{\tikzsetnextfilename{mhd_bperp_0}\input{fig/mhd_bperp_0.tex}} 
\end{figure}
}
\only<3-3>{
\begin{figure}[htbp] 
\resizebox{0.5\linewidth}{!}{\tikzsetnextfilename{mhd_bperp_1}\input{fig/mhd_bperp_1.tex}} 
\end{figure}
}
\only<4-4>{
\begin{figure}[htbp] 
\resizebox{0.5\linewidth}{!}{\tikzsetnextfilename{mhd_bperp_2}\input{fig/mhd_bperp_2.tex}} 
\end{figure}
}
\only<5-6>{
\begin{figure}[htbp] 
\resizebox{0.5\linewidth}{!}{\tikzsetnextfilename{mhd_bperp}\input{fig/mhd_bperp.tex}} 
\end{figure}
}
...
\item Rotation angle: $\psi = \arctan\left(B_z/B_y\right)$.
\item Twist angle: $\alpha = \psi_r - \psi_l$.
\item Solution: waves separating distinct states.
\ebi
}
%% \only<2-2>{
%% \bei
...
\only<7->{
\begin{figure}[htbp] 
\resizebox{0.4\linewidth}{!}{\tikzsetnextfilename{bfield_init_coplanar}\input{fig/bfield_init_coplanar.tex}} 
\end{figure}
}
\only<7->{
\bei
\item Coplanar magnetic field
\ebi
}
\end{frame}
...
\pause
\item Satisfy jump conditions (coplanar case).
\ebi
\ebi
}
%% \only<3-3>{
...
%----------------------------------------------------------------------------------------
\begin{frame}[fragile]
\frametitle{Pseudo-convergence}
\only<1-1>{
\begin{figure}[htbp]
\resizebox{0.5\linewidth}{!}{\tikzsetnextfilename{bfield_init_coplanar_b}\input{fig/bfield_init_coplanar_b.tex}}
\end{figure}
}
\only<2->{
\begin{tabular}{cc}
...
\pause
\item Pseudo-convergence: rotational discontinuity at higher resolutions \citep{Torrilhon:2003b}.
...
% CWM
%----------------------------------------------------------------------------------------
\section{Removing compound wave formation in ideal MHD}
...
\only<1->{
\bei
\item Need to reduce numerical diffusion. %for near coplanar problems.
\pause
\only<1-3>{
...
\pause
\item Calculate flux $\mbf{F}^c$ between $\mbf{U^*_{l}}$ and $\mbf{U}^*_{2l}$.
\pause 
...
\only<5->{
\begin{tabular}{cc}
\resizebox{0.35\linewidth}{!}{\tikzsetnextfilename{coplanar_a_rsol_init_fct_1}\input{fig/coplanar_a_rsol_init_fct_1.tex}} & 
\resizebox{0.35\linewidth}{!}{\tikzsetnextfilename{coplanar_a_rsol_init_fct_6} \input{fig/coplanar_a_rsol_init_fct_6.tex}}
\end{tabular}
}
\end{center}
...
\bei
\item Works with FCT!
\item At rotation, set antidiffusion = CWM.
\item Larger Cour. No. without additional diffusion downstream.
\item Slow shock downstream poorly resolved, but weak.
\ebi
}
\end{frame}
...
\only<1-1>{
\includemovie[
  label=kh_instability
  activate=onclick,
  %% controls=false,
text={\includegraphics[width=\textwidth]{kh0.jpg}}]{\textwidth}{}{kh_instability_10_fps.avi}
}
\only<2->{
\includegraphics[width=\textwidth]{kh1.jpg}
}
\end{column}
...
\caption{Performance comparison for Kelvin-Helmholtz Instability.} %\citep{Orszag:1979}.}
...
%% \item  with a memory bandwidth of 288.4 GB/sec and 2688 CUDA cores.
