...
\pause
\item Plasma flow becomes subsonic forming the bow sbock.
\ebi
}
\only<2->{
...
\only<3->{
\item Flux corrected transport (FCT).
...
\only<3-3>{
\bei
\item An intermediate shock is a non-regular, i.e., not a Lax shock, wave that changes the orientation of the magnetic field.
...
\item As $\alpha \rightarrow \pi$, grid resolution increases. 
